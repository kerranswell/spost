...
class VKPublic
{
    ...
    public function callPostMethod($method, $parameters)
    {
        if (!$this->accessToken) return false;

        if (is_array($parameters)) $sp = http_build_query($parameters);
        $queryString = "/method/$method?$sp&access_token={$this->accessToken}";
        $querySig = md5($queryString . $this->accessSecret);

        $ch = curl_init('http://api.vk.com/method/'.$method);
        curl_setopt($ch, CURLOPT_HEADER, false);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $parameters['access_token'] = $this->accessToken;
        $parameters['sig'] = $querySig;
    ...
    {
        return $this->callPostMethod('wall.post', array(
     ...
     * @return mixed
     */
    public function wallPostAttachment($attachment = null, $message = null, $publish_date = false, $fromGroup = true, $signed = false)
    {
        $ar = array(
            'owner_id' => -1 * $this->groupId,
            'message' => $message,
        ...
        if ($attachment != null) $ar['attachment'] = strval($attachment);
        if ($publish_date > 0) $ar['publish_date'] = $publish_date;
        return $this->callPostMethod('wall.post', $ar);
    }
